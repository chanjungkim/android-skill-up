# android-skill-up
For android skill up

# Codelabs

[ViewModel](https://developer.android.com/codelabs/kotlin-android-training-view-model#0)
[LiveData](https://developer.android.com/codelabs/kotlin-android-training-view-model#0)
[Coroutine](http://sjava.net/2016/05/번역-안드로이드-앱이-메모리-누수leak를-만드는-8가지/)

# 형상 관리

[Git Flow](https://danielkummer.github.io/git-flow-cheatsheet/index.ko_KR.html)

# 코드 스타일

[Android Kotlin style guide](https://developer.android.com/kotlin/style-guide)
[Kotlinlang coding conventions](https://kotlinlang.org/docs/coding-conventions.html)

# 메모리

[메모리 구조](https://twinparadox.tistory.com/614)
[메모리 누수](http://sjava.net/2016/05/번역-안드로이드-앱이-메모리-누수leak를-만드는-8가지/)